...
#--------------------------Impelment your code here:-------------------------------------
new_a_model = Sequential(name="model_sigmoid")
...
history_2 = new_a_model.fit(BaseX_train, BaseY_train, batch_size=batch_size, epochs=epochs, 
                         verbose=2, validation_data=(X_test, Y_test))

if not("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "model_sigmoid.h5"
model_path = os.path.join(save_dir, model_name)
new_a_model.save(model_path)

loss_and_metrics_2 = new_a_model.evaluate(X_test, Y_test, verbose=2)
print("Test Loss is {:.2f} ".format(loss_and_metrics_2[0]))
print("Test Accuracy is {:.2f} %".format(100*loss_and_metrics_2[1]))
#-----------------------------------------------------------------------------------------
...
#--------------------------Impelment your code here:-------------------------------------
new_a_model = load_model("results/model_sigmoid.h5")

history_3 = new_a_model.fit(BaseX_train, BaseY_train, batch_size=batch_size, epochs=epochs, 
                         verbose=2, validation_data=(X_test, Y_test))

loss_and_metrics_3 = new_a_model.evaluate(X_test, Y_test, verbose=2)
print("Test Loss is {:.2f} ".format(loss_and_metrics_3[0]))
print("Test Accuracy is {:.2f} %".format(100*loss_and_metrics_3[1]))
#-----------------------------------------------------------------------------------------
...
history_4 = model_relu.fit(BaseX_train, BaseY_train, batch_size=batch_size, epochs=epochs, 
                         verbose=2, validation_data=(X_test, Y_test))

loss_and_metrics_4 = model_relu.evaluate(X_test, Y_test, verbose=2)
print("Test Loss is {:.2f} ".format(loss_and_metrics_4[0]))
print("Test Accuracy is {:.2f} %".format(100*loss_and_metrics_4[1]))
#----------------------------------------------------------------------------------------
...
#--------------------------Impelment your code here:-------------------------------------
history_5 = new_a_model.fit(BaseX_train, BaseY_train, batch_size=batch_size, epochs=epochs, 
                         verbose=2, validation_data=(X_test, Y_test))

loss_and_metrics_5 = new_a_model.evaluate(X_test, Y_test, verbose=2)
print("Test Loss is {:.2f} ".format(loss_and_metrics_5[0]))
print("Test Accuracy is {:.2f} %".format(100*loss_and_metrics_5[1]))
#----------------------------------------------------------------------------------------
...
#--------------------------Impelment your code here:-------------------------------------
num_fil = [32, 64, 64, 128, 128]

def get_net(input_shape,num_fil,drop,dropRate,reg):
    #Defining the network architecture:
    model = Sequential()
    model.add(Permute((1,2,3),input_shape = input_shape))
    
    for i, fil in enumerate(num_fil):
        model.add(Conv2D(filters=fil, kernel_size=(3,3), padding='same', 
...
keras.backend.clear_session()
NNet_2 = get_net(input_shape,num_fil,drop,dropRate,reg)
NNet_2.compile(optimizer=AdamOpt, metrics=['acc'], loss='categorical_crossentropy')
h_2 = NNet_2.fit(x=BaseX_train, y=BaseY_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_split=0, validation_data = (BaseX_val, BaseY_val), shuffle=True)
